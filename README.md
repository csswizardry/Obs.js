...
| `.has-delivery-mode-cautious`           | Be careful/middle weight | **Otherwise** (not `rich`/`lite`). Examples: `batteryLow === true` (without `dataSaver` or `batteryCritical`), or `connectionCapability === 'moderate'`. |
...
| `deliveryMode`           | `'rich' \| 'cautious' \| 'lite'`   | How ‘heavy’ you should go                     | Derived from capability and user/device signals                | **rich** if connection is **strong** and **not** (`dataSaver` **or** `batteryCritical`); **lite** if **weak** **or** `dataSaver` **or** `batteryCritical`; otherwise **cautious** |
